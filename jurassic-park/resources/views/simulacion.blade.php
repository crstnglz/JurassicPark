...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .resumen-card { background: rgba(0,0,0,0.6); border-radius: 12px; padding: 20px; text-align: center; }
        ...
        .celda-card { background: rgba(0,0,0,0.6); border-radius: 12px; padding: 20px; transition: all 0.2s; }
        ...
        .btn-asignar { width: 100%; background: #6366f1; color: white; border: none; padding: 8px; border-radius: 8px; cursor: pointer; font-weight: bold; margin-top: 12px; }
...
<div class="navbar">
    ...
    <div class="nav-right">
        ...
        <button class="btn-nav red" onclick="window.location.href='/simulacion-brecha'">💥 Brecha</button>
...
<div class="container">
    ...
    <div class="sim-box">
        ...
        <button class="btn-sim" id="btnSim" onclick="lanzarSimulacion()">🚀 Iniciar Simulación</button>
    </div>
...
function mostrarResultados(data) {
    ...
    document.getElementById("informeGrid").innerHTML = informe.map(c => `
        <div class="celda-card ${c.alerta}">
            <h4 class="${c.alerta}">
                ${c.alerta === 'critica' ? '🚨' : c.alerta === 'atencion' ? '⚠️' : '✅'} ${c.nombre}
            </h4>
            ...
            <div style="margin-top:10px">
                ...
                <div id="trabajadores-${c.id}"><span style="color:#6b7280; font-size:13px">Cargando...</span></div>
            </div>
            <button class="btn-asignar" onclick="openModal(${c.id}, '${c.nombre}')">👷 Asignar Trabajador</button>
        </div>
    `).join('')
...
function loadTareasCelda(celdaId) {
    ...
    .then(res => res.json())
    .then(tareas => {
        const tareasCelda = tareas.filter(t => t.celda_id == celdaId && t.estado !== 'completada')
...
function saveAsignar() {
    ...
    fetch('/api/tareas', {
        ...
        body: JSON.stringify({ user_id, celda_id: celdaSeleccionada, tipo, descripcion: 'Asignado tras simulación normal' })
    })
    .then(res => res.json())
    .then(data => {
        if (data.success) {
            showToast('✅ Trabajador asignado correctamente', 'success')
            closeModal()
            loadTareasCelda(celdaSeleccionada)
        } else {
            showToast(data.message || "Error al asignar", 'danger')
        }
    })
}
