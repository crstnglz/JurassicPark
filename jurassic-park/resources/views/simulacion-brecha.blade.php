...
<div class="container">
    ...
    <div class="sim-box">
        ...
        <div class="selector-row">
            <div>
                ...
                <select id="selectCelda">
                    <option value="">🎲 Aleatoria</option>
                </select>
            </div>
            <button class="btn-sim-brecha" id="btnBrecha" onclick="lanzarBrecha()">
                💥 Lanzar Brecha
            </button>
        </div>
    </div>
    ...
    <div id="resultadoPanel" style="display:none">
        ...
        <div class="informe-grid">
            <div class="info-card">
                ...
                <div class="stat">
                    ...
                    <span class="val" id="iNombre"></span>
                </div>
                <div class="stat">
                    <span class="lbl">Seguridad</span>
                    <span id="iSeguridad"></span>
                </div>
                <div class="stat">
                    <span class="lbl">Peligrosidad</span>
                    <span id="iPeligrosidad"></span>
                </div>
                <div class="stat">
                    <span class="lbl">🍖 Alimento</span>
                    <span class="val" id="iAlimento"></span>
                </div>
                <div class="stat">
                    <span class="lbl">🔧 Averías</span>
                    <span class="val" id="iAverias"></span>
                </div>
                <div class="stat">
                    <span class="lbl">🦖 Dinosaurios</span>
                    <span class="val" id="iDinos"></span>
                </div>
            </div>
            ...
            <div class="info-card">
                <h4>📊 Estadísticas</h4>
                <div class="stat">
                    <span class="lbl">Probabilidad fuga</span>
                    <span class="val" id="iProb"></span>
                </div>
                <div class="progress-bar">
                    <div class="progress-fill" id="iProbBar"></div>
                </div>
                <div class="stat" style="margin-top:10px">
                    <span class="lbl">💀 Bajas personal</span>
                    <span class="val danger" id="iBajas"></span>
                </div>
                <div class="stat">
                    <span class="lbl">🩸 Dinos heridos</span>
                    <span class="val warn" id="iDinosHeridos"></span>
                </div>
                <div class="stat">
                    <span class="lbl">🦖 Carnívoros sueltos</span>
                    <span class="val danger" id="iCarnivoros"></span>
                </div>
                <div class="stat">
                    <span class="lbl">😡 Dinos agresivos</span>
                    <span class="val warn" id="iAgresivos"></span>
                </div>
            </div>
        </div>
        ...
        <div class="acciones-box" id="accionesBox" style="display:none">
            ...
            <button class="btn-asignar" onclick="openModal()">
                👷 Asignar Trabajador a la Celda
            </button>
        </div>
...
function lanzarBrecha() {
    ...
    const payload = celdaId ? { celda_id: celdaId } : {}
...
function mostrarResultado(informe) {
    ...
    const titulos = {
        contenida:  'Brecha Contenida',
        fuga_menor: 'Fuga Menor',
    ...
    document.getElementById("resultadoProb").innerText  =
        `Probabilidad de fuga calculada: ${informe.probabilidad_fuga}%`

    // Datos celda
    document.getElementById("iNombre").innerText   = informe.celda.nombre
    document.getElementById("iSeguridad").innerHTML =
        `<span class="badge badge-${informe.celda.nivel_seguridad}">${informe.celda.nivel_seguridad}</span>`
    document.getElementById("iPeligrosidad").innerHTML =
        `<span class="badge badge-${informe.celda.nivel_peligrosidad}">${informe.celda.nivel_peligrosidad.replace('_',' ')}</span>`
    ...
    if (informe.resultado !== 'contenida') {
        const accionesBox  = document.getElementById("accionesBox")
        const accionesDesc = document.getElementById("accionesDesc")
        accionesBox.style.display = "block"

        if (informe.resultado === 'catastrofe') {
            accionesDesc.innerText = '🚨 Situación crítica — asigna veterinarios y mantenimiento urgentemente a esta celda.'
        } else {
            accionesDesc.innerText = '⚠️ Fuga menor detectada — se recomienda asignar personal para revisar la celda.'
        }
...
function saveAsignar() {
    ...
    if (!user_id) { alert("Selecciona un trabajador"); return }
    ...
    fetch('/api/tareas', {
        ...
        body: JSON.stringify({
            user_id:     user_id,
            celda_id:    celdaAfectadaId,
            tipo:        tipo,
            descripcion: 'Asignado tras brecha de seguridad'
        })
    })
    .then(res => res.json())
    .then(data => {
