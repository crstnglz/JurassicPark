...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .tarea-card { background: rgba(0,0,0,0.6); border-radius: 12px; padding: 20px; transition: all 0.2s; }
        ...
        .form-box { background: rgba(0,0,0,0.7); border: 1px solid #facc15; border-radius: 12px; padding: 25px; margin-top: 30px; }
        ...
        input, select, textarea { width: 100%; padding: 10px; background: #020617; border: 1px solid #14532d; color: white; border-radius: 8px; }
    ...
    <div class="form-box" id="formBox" style="display:none">
        ...
        <div class="form-row">
            <div>
                ...
                <select id="fUser"><option value="">Selecciona trabajador</option></select>
            </div>
            <div>
                <label>Celda</label>
                <select id="fCelda"><option value="">Selecciona celda</option></select>
            </div>
        </div>
...
function renderTareas(tareas) {
    ...
    if (!tareas.length) { grid.innerHTML = '<div class="empty">📋 No hay tareas asignadas aún</div>'; return }
...
function saveForm() {
    ...
    if (!payload.user_id || !payload.celda_id) { showToast("Selecciona un trabajador y una celda", 'warning'); return }
    ...
    .then(res => res.json())
    .then(data => {
        if (data.success) { showToast(data.message, 'success'); hideForm(); getTareas() }
        else showToast(data.message || "Error al asignar tarea", 'danger')
    })
}

function deleteTarea(id) {
    if (!confirm("¿Eliminar esta tarea?")) return
    fetch(`/api/tareas/${id}`, { method: 'DELETE', headers: { 'Authorization': 'Bearer ' + token } })
    .then(res => res.json())
    .then(data => { showToast(data.message, 'success'); getTareas() })
}
